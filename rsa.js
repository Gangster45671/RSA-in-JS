class RSAKey {
    constructor(generate) {
        if (generate) {
            ...
            this.p = primes[Math.floor(Math.random() * (primes.length - 1) - (0.9 * (primes.length - 1)) + 1 + (0.9 * (primes.length - 1)))];
            this.q = primes[Math.floor(Math.random() * (primes.length - 1) - (0.9 * (primes.length - 1)) + 1 + (0.9 * (primes.length - 1)))];
    ...
    importPrivate(key, n) {
        ...
        console.log("SET AS PRIVATE KEY:");
        console.log("\t d = " + this.d);
        console.log("\t n = " + this.n);
    }
...
class RSA {
    ...
    static encryptWithKey(text, keyObject) {
        ...
        //read key
        if (keyObject.constructor.name != "RSAKey") {
            throw ("[RSA] You must pass a RSAKey object!");
        }
        //encrypt
        var cipherText = [];
        text = RSA.StringToCharArray(text);
        for (var i = 0; i < text.length; i++) {
            cipherText.push((RSA.modExp(text[i], keyObject.key, keyObject.n)).toString(36).toUpperCase());
        }
    ...
    //advanced math
    static modExp(a, b, n) { // a^b modExp n
        a = a % n;
        var result = 1;
        var x = a;
        while (b > 0) {
...
function testCompPower() {
    //start tests
    //document.write("Testing Encryption/Decryption...<br />");
    var key = new RSAKey(true);
    //encrypt message and send to friends
    key.setType("public");
    var message = RSA.encryptWithKey("hello", key);
    // console.log(message);
    //decrypt message from friends
    key.setType("private");
    message = RSA.decryptWithKey(message, key);
    // console.log(message);
    if (message == "hello") {
        //document.write("TEST OK<br /><br />");
    } else {
        console.error("Not enough computational power or calculational error!");
        return false;
    }
    ...
    //document.write("Testing Signatures...<br />");
    var MEkey = new RSAKey(true);
    //sign message
    key.setType("private");
    var message = RSA.encryptWithKey("this is from me", key);
    // console.log(message);
    //decrypt message to verify sign
    key.setType("public");
    message = RSA.decryptWithKey(message, key);
    // console.log(message);
    if (message == "this is from me") {
        //document.write("TEST OK<br /><br />");
    } else {
        console.error("Not enough computational power or calculational error!");
        return false;
